...
**Last Updated:** 2026-07-30 (v0.1.2 released end-to-end — 1.5E/1.5F code shipped, plus a CI-audit `--locked` fix, advisory dep bumps, and an embedded-materialize race fix; see 2026-07-30 SESSION PROGRESS in [context.md](qli-foundation-context.md). Same day: **Phase 1.5 fully closed** — curl `self-update` live-verified in a sandbox, and the 1.5E/1.5F session verifies executed via the `claude` CLI (headless `-p` slash-command dispatch + `claude plugin marketplace add`/`install` from a clean state, restored afterward). All Phase 1.5 acceptance rows now checked.)
...
- [x] **Block PR merge on CI green.** Applied 2026-05-04 after repo flipped public in 1.5A. Required checks (9): `rustfmt`, `clippy (ubuntu-latest)`, `clippy (macos-14)`, `test (ubuntu-latest)`, `test (macos-14)`, `build (ubuntu-latest)`, `build (macos-14)`, `cargo-audit`, `plan` (release.yml's PR-time dist sanity check). Settings: `strict` (must be up-to-date with main), `enforce_admins: false` (owner keeps direct-push flow; rules apply only to non-admin contributors), no PR review requirement (solo author), no force-pushes, no deletion. Applied via `gh api repos/QLangstaff/qli/branches/main/protection --method PUT` (full JSON body in [context.md](qli-foundation-context.md) SESSION PROGRESS for 2026-05-04).
...
- [x] Created [`QLangstaff/homebrew-qli`](https://github.com/QLangstaff/homebrew-qli) (2026-05-04). Public, README + MIT, default branch `main`. The `homebrew-` prefix is mandatory for Homebrew's auto-tap to find it; `brew install QLangstaff/qli/qli` resolves to that repo.
- [x] Configured cargo-dist to publish to it on release (2026-05-04). Two keys added to [`dist-workspace.toml`](../../../dist-workspace.toml): `tap = "QLangstaff/homebrew-qli"` + `publish-jobs = ["homebrew"]`. Without the second key the formula is built as a release artifact (1.5A state) but never committed to the tap. Re-ran `dist generate` to update [`.github/workflows/release.yml`](../../../.github/workflows/release.yml) with the new `publish-homebrew-formula` job (cargo-dist's CI runs `dist generate --check` and would fail the build if the YAML drifted from config). The new job: checks out the tap repo using `${{ secrets.HOMEBREW_TAP_TOKEN }}`, downloads the `qli.rb` artifact, runs `brew style --fix`, commits to `Formula/qli.rb`, pushes. The `announce` job now waits on it.
- [x] PAT + secret in place (2026-05-04). Fine-grained PAT scoped to `QLangstaff/homebrew-qli` with Contents: read-and-write + Metadata: read; stored as `HOMEBREW_TAP_TOKEN` on `QLangstaff/qli`. Fine-grained over classic-`repo` scope deliberate per advisor — classic `repo` would grant write to every repo on the account, including this one.
- [x] **Verify** (2026-05-04): v0.1.1 tagged + released; `publish-homebrew-formula` job committed [`Formula/qli.rb`](https://github.com/QLangstaff/homebrew-qli/blob/main/Formula/qli.rb) to the tap as commit `b074c44 qli 0.1.1`. `brew install QLangstaff/qli/qli` on this Mac auto-tapped (no manual `brew tap` needed) and installed to `/opt/homebrew/Cellar/qli/0.1.1/bin/qli`. `qli --version` reports `qli 0.1.1`. End-to-end pipeline real: tag → 5-target build → release → tap publish → user-installable.
...
- [x] Detect install method in [`crates/qli/src/self_update.rs`](../../../crates/qli/src/self_update.rs): canonicalize both `current_exe()` and `$HOME`, then match in order — `/Cellar/` segment → Homebrew; `$HOME/.cargo/bin/` → cargo if `"qli "` line present in `$HOME/.cargo/.crates.toml`, else curl; `$HOME/.local/bin/` → curl alt; else Unknown.
- [x] For curl-installed: re-exec `qli-installer.sh` via `sh -c 'curl -LsSf <installer-url> | sh'`. On `cfg(target_os = "windows")` the curl branch falls through to "print upgrade command" (Windows installer is `.ps1`).
- [x] For Homebrew: prints `Upgrade qli with: brew upgrade qli`, exit 0.
- [x] For cargo: prints `Upgrade qli with: cargo install qli --force`, exit 0.
- [x] For Claude Code plugin: appended `Claude Code plugin? Refresh it with /plugin marketplace update qli-plugins …` note on every output (cargo/brew/curl/unknown alike).
- [x] Verify: smoke against release binary in `/tmp/qli-su-smoke/` with three fake install layouts (cargo with `.crates.toml`, brew under `Cellar/`, unknown outside any branch). Each: right upgrade command + plugin note + exit code (0 for cargo/brew, 1 for unknown). `--json` shape: `{install_method, binary_path, upgrade_command, status, plugin_note}` with status `performed|hinted|failed`. **Curl path now live-verified (2026-07-30)**: staged the release binary in an isolated `HOME`/`CARGO_HOME` sandbox as a curl-layout install (`.cargo/bin/qli`, no `.crates.toml`), ran `qli self-update` — detected `curl`, re-exec'd the real `qli-installer.sh`, downloaded + installed `qli 0.1.2`, `--json` reported `install_method:"curl" status:"performed"`, human output printed `qli upgraded via …/qli-installer.sh` + plugin note, exit 0; real `~/.cargo/bin/qli` untouched.
- [x] Exit-code contract change: stub exited 2 (USAGE); 1.5E exits 0 for performed/hinted, 1 for failed/unknown. Plugin-contract test [`tests/claude_code_plugin.rs::self_update_unknown_install_method_exits_1`](../../../crates/qli/tests/claude_code_plugin.rs) updated to assert the new contract (the `target/debug/qli` test binary lands in the unknown branch).
- [x] **Plugin update**: edit [`claude-code-plugin/skills/qli/SKILL.md`](../../../claude-code-plugin/skills/qli/SKILL.md) "When NOT to invoke" bullet — dropped the "stub … prints not implemented" line; replaced with passive "do not autonomously run, but shell via Bash when the user explicitly asks to upgrade qli." (Also refreshed the stale "exits 2 (USAGE) … stub message" line under Output discipline to the real 0/1 contract.) Landed in the v0.1.2 release commit `6c2ccfa` (2026-07-30).
- [x] **Bump plugin version pins** (3 files: `plugin.json`, `SKILL.md` "Targets qli vX.Y.Z", `claude-code-plugin/README.md` "Targets qli vX.Y.Z") + workspace `Cargo.toml` (0.1.1 → 0.1.2) + qli-ext path-dep version + `crates/qli/tests/cmd/version.toml` snapshot — all in release commit `6c2ccfa` (2026-07-30). `cargo test -p qli --test cli` + full workspace gate run green before push; plugin-contract test [`plugin_version_matches_qli_crate_version`](../../../crates/qli/tests/claude_code_plugin.rs) passes with all pins at 0.1.2.
- [x] **Verify** (2026-07-30): headless `claude -p --plugin-dir ./claude-code-plugin --model haiku --allowedTools Bash`. `/qli:ext-list` still dispatches after the SKILL.md edit (returned the 3 XDG extensions). The self-update NL question ("how do I upgrade qli?") returned guidance matching the new SKILL.md — `qli self-update` re-execs the installer on the curl path, prints the upgrade command for cargo/brew, writes to stderr, exits 0/1 — with no trace of the old "stub"/"not implemented" wording.
...
- [x] Created [`/.claude-plugin/marketplace.json`](../../../.claude-plugin/marketplace.json) at the qli repo root (sibling to `claude-code-plugin/`, not nested inside it) with the exact field set locked in [context.md → "Plugin install-distribution"](qli-foundation-context.md#architectural-decisions). `$schema` omitted — Claude Code ignores the field at load time and no working marketplace-schema URL exists today. Validated as JSON via `python3 -m json.tool`.
- [x] Updated [`claude-code-plugin/README.md`](../../../claude-code-plugin/README.md) "Regular use" section: replaced the placeholder with the two-step install — `/plugin marketplace add QLangstaff/qli` then `/plugin install qli@qli-plugins`. "Local development" `claude --plugin-dir` subsection preserved underneath.
- [x] **Commit and push to `origin/main`.** Marketplace manifest + README shipped in commit `65e14eb` ("1.5F: plugin marketplace install path") and pushed to `origin/main` 2026-07-30, so `/plugin marketplace add QLangstaff/qli` (a `git clone`) can now see them.
- [x] **Verify end-to-end** (2026-07-30, via `claude` CLI from a clean state — no prior `qli-plugins` marketplace configured): `claude plugin marketplace add QLangstaff/qli` cloned `https://github.com/QLangstaff/qli.git` and populated `~/.claude/plugins/marketplaces/qli-plugins/` (dir named after the marketplace `name` field, not the repo slug, as predicted). `claude plugin install qli@qli-plugins` installed the plugin (v0.1.2, user scope, enabled). All four 1.5D slash commands dispatched through the installed marketplace plugin in headless sessions: `/qli:ext-list` (3 extensions), `/qli:ext-which dev hello` (path), `/qli:ext-install-defaults` (wrote 0/skipped 6 — idempotent), `/qli:completions zsh` (ran `qli completions zsh`). Origin labels read `xdg` (not `embedded` as in the 2026-05-11 run) because install-defaults has since materialized XDG copies that win over embedded — expected; dispatch mechanics identical. Marketplace + plugin removed afterward to restore the original clean state.

### Phase 1.5 acceptance

- [x] All four install paths (cargo, brew, curl, Claude Code plugin slash commands) work. MCP-tool path moved to Phase 5 acceptance. cargo verified in 1.5C, brew in 1.5B, curl in 1.5A + curl `self-update` re-exec live-verified in 1.5E (2026-07-30); the plugin slash-command path verified 2026-07-30 via both the installed marketplace plugin (1.5F) and dev-mode `--plugin-dir` (1.5E) — all four `/qli:*` commands dispatched. (Verified on this machine with fresh/isolated state, not a literally pristine OS.)
- [x] Plugin's "regular use" install path (1.5F outcome — marketplace) works following only the [`claude-code-plugin/README.md`](../../../claude-code-plugin/README.md) install instructions; no `--plugin-dir` dev flag required. Verified 2026-07-30 from a clean state (`qli-plugins` marketplace absent beforehand): `claude plugin marketplace add QLangstaff/qli` → `claude plugin install qli@qli-plugins` → all four slash commands dispatched, then state restored. See 1.5F verify.
- [x] Tagged release produces all artifacts automatically. v0.1.2 tag → `release.yml` green: 5-target build, GitHub release (16 assets), Homebrew tap formula → 0.1.2, and (local) all 5 crates published to crates.io at 0.1.2 (2026-07-30).
- [x] `qli self-update` works for the curl-installed path and prints correct guidance for the others. Curl path live-verified 2026-07-30 (sandboxed re-exec of `qli-installer.sh` → downloaded + installed `qli 0.1.2`, exit 0; see 1.5E smoke bullet). cargo/brew/unknown guidance smoke-verified in 1.5E.
