...
- [ ] Verify: `brew install QLangstaff/qli/qli` on a clean machine installs the latest version. _Pending v0.1.1 release: tap-publish jobs only fire on the next release after the config landed; v0.1.0 does not appear in the tap. Will verify on this machine via `brew untap` (defensive) → `brew install` → `qli --version` reports `0.1.1`._
