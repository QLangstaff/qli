...
- [x] Verify: smoke against release binary in `/tmp/qli-su-smoke/` with three fake install layouts (cargo with `.crates.toml`, brew under `Cellar/`, unknown outside any branch). Each: right upgrade command + plugin note + exit code (0 for cargo/brew, 1 for unknown). `--json` shape: `{install_method, binary_path, upgrade_command, status, plugin_note}` with status `performed|hinted|failed`. Curl path not live-smoked (re-exec would actually install) — unit tests cover detection; live-curl verify lands naturally next time the user releases.
...
- [ ] **Plugin update**: edit [`claude-code-plugin/skills/qli/SKILL.md`](../../../claude-code-plugin/skills/qli/SKILL.md) "When NOT to invoke" bullet — drop the "stub … prints not implemented" line; replace with passive "do not autonomously run, but shell via Bash when the user explicitly asks to upgrade qli." **Apply together with the release commit** (next bullet). SKILL.md on `main` must accurately describe the binary version it targets; landing this edit ahead of the version bump would misrepresent v0.1.1 binaries.
- [ ] **Bump plugin version pins** (3 files: `plugin.json`, `SKILL.md` "Targets qli vX.Y.Z", `claude-code-plugin/README.md` "Targets qli vX.Y.Z") + workspace `Cargo.toml` (0.1.1 → 0.1.2) + qli-ext path-dep version + `crates/qli/tests/cmd/version.toml` snapshot, all in one release commit. Run `cargo test -p qli --test cli` locally before pushing (the snapshot whack-a-mole from 1.5A/1.5B is exactly this). Plugin-contract test [`plugin_version_matches_qli_crate_version`](../../../crates/qli/tests/claude_code_plugin.rs) currently asserts all pins match the workspace version.
- [ ] **Verify (user-driven)**: in a fresh `claude --plugin-dir ./claude-code-plugin` session, run `/qli:ext-list` (sanity-check existing dispatch still works after the SKILL.md edit) and confirm Claude's response to a self-update question (e.g., "how do I upgrade qli?") reflects the new SKILL.md guidance rather than the old "stub" wording.
...
- [ ] **Commit and push to `origin/main`.** `/plugin marketplace add QLangstaff/qli` does a `git clone` from GitHub and cannot see local-only changes; the marketplace manifest + README edits must be on `main` before the verify step runs. **Deferred to user** (file work landed in working tree this session; user controls release timing).
- [ ] Verify end-to-end on a fresh Claude Code session with no prior `~/.claude/plugins/marketplaces/qli-plugins/` entry: run `/plugin marketplace add QLangstaff/qli`, confirm `~/.claude/plugins/marketplaces/qli-plugins/` is populated (named after the marketplace's `name` field, not the GitHub repo slug — pattern confirmed via `~/.claude/plugins/known_marketplaces.json`), run `/plugin install qli@qli-plugins`, dispatch all four 1.5D slash commands (`/qli:ext-list`, `/qli:ext-which dev hello`, `/qli:ext-install-defaults`, `/qli:completions zsh`) and confirm output matches the 2026-05-11 1.5D verify run. **Deferred to user** — fresh Claude Code session.
