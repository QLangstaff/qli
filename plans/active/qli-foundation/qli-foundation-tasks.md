...
- [x] Added [`.github/workflows/ci.yml`](../../../.github/workflows/ci.yml) with five jobs: `fmt` (Linux only — `cargo fmt --all -- --check`), `clippy` (matrix — `cargo clippy --workspace --all-targets -- -D warnings`), `test` (matrix — `cargo test --workspace`), `build` (matrix — `cargo build --release --workspace`), and `audit` (Linux only — `rustsec/audit-check@v2`). Build matrix is `ubuntu-latest` (Linux x86_64) + `macos-14` (macOS arm64); Windows excluded from 1K — cargo-dist handles cross-compile in Phase 1.5A. Triggers: `push` on `main`, all `pull_request`s, and `merge_group` (so the workflow runs in GitHub's merge queue if the user enables it later). `permissions: contents: read` at workflow level keeps the default token narrow.
- [x] `cargo audit` job uses `rustsec/audit-check@v2`. **Deliberate simplification**: the action fails CI on **any** RustSec advisory, not on the "≥ medium" tier the task originally specified. RustSec advisories don't carry consistent CVSS severity, so a tier filter would require either custom JSON parsing of `cargo audit --json` or adopting `cargo-deny` for its `[advisories] severity-threshold` knob. The plan ([qli-foundation-plan.md:64](qli-foundation-plan.md)) explicitly chose standalone `cargo audit` over `cargo-deny`; matching that stance here. If advisory noise becomes a real problem, revisit by switching to `cargo-deny` rather than building parallel JSON-filter logic.
- [x] Caching is provided by `actions-rust-lang/setup-rust-toolchain@v1`, which (a) honors `rust-toolchain.toml` so the Rust version is single-sourced from the pin file (bump the pin to bump CI), and (b) bundles `Swatinem/rust-cache@v2` internally — cache key derives from `Cargo.lock` + toolchain version. Each job runs `rustup show active-toolchain` as a sanity check so a misconfigured runner fails loudly with the version it's actually using.
- [ ] **Block PR merge on CI green.** Deferred: branch protection on private repos requires GitHub Pro on a paid plan; the repo is private on free, and the project's "no paid services" constraint ([context.md:233](qli-foundation-context.md)) rules out an upgrade purely for this. Attempted via `gh api repos/QLangstaff/qli/branches/main/protection --method PUT` on 2026-05-03 → 403 `Upgrade to GitHub Pro or make this repository public`. Re-enable when the repo goes public (likely Phase 1.5 — cargo-dist / Homebrew tap / Claude Code plugin all need a public repo). Required status checks to enable at that point: `rustfmt`, `clippy (ubuntu-latest)`, `clippy (macos-14)`, `test (ubuntu-latest)`, `test (macos-14)`, `build (ubuntu-latest)`, `build (macos-14)`, `cargo-audit`.
- [ ] **Verify the gates fail when they should** (pending; not blocking 1K acceptance): a deliberate clippy violation fails CI; reverting passes. A pinned crate with a known advisory fails the audit job. The "gates succeed on a clean tree" half of the verify is empirically green — run [25282714217](https://github.com/QLangstaff/qli/actions/runs/25282714217) shows all 8 jobs (rustfmt, clippy ×2, test ×2, build ×2, cargo-audit) pass. The "gates fail when they should" half is reasonable to leave for the user's first real PR or a deliberate red-build experiment.

### 1L: Tests

- [x] **Fixture root.** Created [`tests/fixtures/README.md`](../../../tests/fixtures/README.md) documenting the workspace-root `tests/fixtures/<lang>/` convention. Per-language subdirs are deliberately NOT created here in Push A — they land with the phase that introduces the language adapter or analyzer (Phase 2H ships Python/TypeScript fixtures for the TODO/FIXME analyzer; 2I extends them for the def/ref extractor; 2.5 adds C#; 2.6 adds Angular). README also calls out the path-discipline rule (workspace-root-relative, via `cargo metadata` or `CARGO_MANIFEST_DIR + ..` — not crate-relative) and what does NOT belong here (per-test scratch tempdirs, generated artifacts, gitignored files).
- [x] **Hermetic test harness.** [`crates/qli-ext/tests/common/mod.rs`](../../../crates/qli-ext/tests/common/mod.rs) provides `XdgSandbox`, an RAII guard that creates a `TempDir`, overrides `HOME` + `XDG_CONFIG_HOME` / `XDG_DATA_HOME` / `XDG_STATE_HOME` / `XDG_CACHE_HOME` to subdirs of the tempdir, and restores prior values on `Drop`. Push B will copy this pattern into `crates/qli/tests/common/mod.rs` for `assert_cmd` integration tests; `OnePassword` is already a trait in qli-ext (`SecretsResolver`) that integration tests stub via `TestResolver` — no new code needed for that part of the task. Smoke test [`crates/qli-ext/tests/xdg_sandbox_smoke.rs`](../../../crates/qli-ext/tests/xdg_sandbox_smoke.rs) asserts overrides apply and restore.
    - **`#[serial_test::serial]` gates added** to all 10 env-mutating unit tests in `qli-ext`: 3 in `audit::tests` (`expand_uses_process_env_first`, `expand_falls_back_to_defaults_when_env_unset`, `expand_errors_on_unset_var_with_no_default`), 1 in `dispatch::tests` (`requires_env_blocks_spawn`), 3 in `secrets::tests` (`env_provider_reads_reference_writes_env`, `env_provider_errors_when_reference_unset`, `production_resolver_dispatches_per_spec_provider`), 3 in `guard::tests` (`check_requires_env_passes_when_match`, `_errors_when_missing`, `_errors_when_mismatched`). Unique-name discipline kept as the first line of defence; `#[serial]` is the second. The deliberate "we do *not* depend on `serial_test`" comment in `secrets.rs` was rewritten to explain the two-layer defence.
    - **Verify (advisor pre-flight):**
        - `XDG_CONFIG_HOME=/nonexistent XDG_DATA_HOME=/nonexistent XDG_STATE_HOME=/nonexistent XDG_CACHE_HOME=/nonexistent cargo test -p qli -p qli-ext` — green (qli 3 unit + qli-ext 60 unit + secrets_never_leak 1 + xdg_sandbox_smoke 1).
        - `cargo test -p qli-ext -- --test-threads=4` — green; `#[serial]` correctly serializes env-mutating tests under the higher concurrency.
        - `cargo clippy --workspace --all-targets -- -D warnings` clean; `cargo fmt --all -- --check` clean.
- [x] **Engine-purity test.** [`crates/qli-core/tests/dependency_purity.rs`](../../../crates/qli-core/tests/dependency_purity.rs) parses `cargo metadata --no-deps` for the qli-core package and asserts every `DependencyKind::Normal` direct dep appears in a hardcoded `ALLOWED_DIRECT_DEPENDENCIES` constant. Constant starts empty (qli-core currently has zero direct deps). `[dev-dependencies]` are not gated — `cargo_metadata` itself is the obvious dev-only addition. Failure message names the offender(s) and points at the plan rule.
    - **Negative-path verify (advisor pre-flight):** added `tracing = "0.1"` to `crates/qli-core/Cargo.toml`, ran the test, confirmed it fails with `Disallowed direct dependencies: ["tracing"]. Either remove them from qli-core/Cargo.toml or add them to ALLOWED_DIRECT_DEPENDENCIES in this test...`. Reverted; positive path passes again. The test bites as designed.
- [x] **CLI contract snapshots.** [`crates/qli/tests/cli.rs`](../../../crates/qli/tests/cli.rs) drives `trycmd` against six case files under [`crates/qli/tests/cmd/`](../../../crates/qli/tests/cmd/): [`version.toml`](../../../crates/qli/tests/cmd/version.toml), [`completions-zsh.toml`](../../../crates/qli/tests/cmd/completions-zsh.toml) (header line + `...` multi-line elision; the script body churns on every clap minor bump), [`completions-help.toml`](../../../crates/qli/tests/cmd/completions-help.toml) (the `COMPLETIONS_AFTER_HELP` examples block — a real 1B contract addition that should be locked), [`unknown-no-tip.toml`](../../../crates/qli/tests/cmd/unknown-no-tip.toml) (`qli foo` → no close match), [`unknown-with-tip.toml`](../../../crates/qli/tests/cmd/unknown-with-tip.toml) (`qli porod hello` → tip suggesting `prod`), [`missing-env.toml`](../../../crates/qli/tests/cmd/missing-env.toml) (`qli prod hello` without `QLI_ENV` → banner + export hint).
    - Root `qli --help` deliberately not snapshotted: the dynamic group/extension synthesis from Phase 1E lists `xdg`/`embedded` paths in subcommand `about` text, which differ between dev machine and CI runner. `qli completions --help` (above) covers the static after_help examples cleanly without that path noise.
    - **Hermeticity**: harness uses a per-invocation `tempfile::TempDir` and `TestCases::env(...)` to point `HOME` + four `XDG_*` vars at it. `trycmd` has no `env_remove`, so the harness wraps the test body in a tiny `EnvUnset` RAII guard for `QLI_ENV` (otherwise a dev with `QLI_ENV` exported would see the missing-env case behave differently). Test gated `#[serial_test::serial]` like every other env-mutating test in this binary.
    - **Verify**: `cargo test -p qli --test cli` green; `TRYCMD=overwrite cargo test -p qli --test cli` regenerates with zero `git diff` output (snapshots are byte-stable).
- [x] **Dispatcher unit + integration tests.** Unit tests for manifest parsing, discovery, and guard evaluation already shipped across 1D–1J (60 unit tests in `qli-ext`); the missing piece was binary-level integration coverage. [`crates/qli/tests/dispatcher.rs`](../../../crates/qli/tests/dispatcher.rs) adds 6 `assert_cmd`-based tests under the [`crates/qli/tests/common/mod.rs`](../../../crates/qli/tests/common/mod.rs) `XdgSandbox` mirror of the qli-ext harness:
    - `dev_hello_runs_clean` — happy path against the embedded `dev` group, exit 0.
    - `prod_without_env_fails_with_export_hint` — `requires_env` failure, exit 1, stderr contains `export QLI_ENV=prod`.
    - `prod_non_tty_without_yes_fails_confirm` — `confirm` failure (assert_cmd's piped stdin is not a TTY by default), exit 1, stderr contains `--yes`.
    - `env_secret_failure_leaves_audit_empty` — `secrets` failure (env-provider, reference unset), exit 1, audit log empty (resolution fails before audit-start, per Phase 1F).
    - `successful_run_writes_audit_start_and_finish` — `audit_log` success path; audit file contains exactly two JSONL records (`event:start`, `event:finish` with `exit_code:0`).
    - `sigint_during_slow_extension_writes_interrupted_audit` — SIGINT integration test. Spawns `qli slow wait` (a `sleep 30` extension) into its own process group via `std::os::unix::process::CommandExt::process_group(0)` (a *safe* stdlib API, stable since Rust 1.64 — equivalent to `setpgid(0,0)` in the child after fork, no `unsafe` needed). Polls audit log until `event:start` lands, sleeps 100 ms, then `killpg(qli_pgid, SIGINT)` simulates the terminal Ctrl+C broadcast: both qli and child receive SIGINT directly, child dies of SIGINT (130), qli propagates `128 + 2 = 130`. Asserts exit code **130** (matches plan task) plus `event:interrupted` audit record. Audit `signal` field reads `SIGTERM` per the Phase 1F simplification (the ctrlc handler always forwards SIGTERM regardless of originating signal); the test asserts the event presence, not the label.
    - **Tooling**: added `assert_cmd = "2"`, `predicates = "3"`, `serial_test = "3"`, `tempfile = "3"` as dev-deps to `qli`, plus unix-only `nix = "0.29"` (signal feature) for the SIGINT test.
    - **Verify**: `cargo test --workspace` runs 73 total tests (was 66 pre-Push-B): qli unit 3 + qli integration 7 (dispatcher 6 + cli 1) + qli-core integration 1 + qli-ext unit 60 + qli-ext integration 2. `cargo clippy --workspace --all-targets -- -D warnings` clean. `cargo fmt --all -- --check` clean. `XDG_CONFIG_HOME=/nonexistent ... cargo test -p qli -p qli-ext` green (nothing reaches host XDG). `cargo test --workspace -- --test-threads=4` green (`#[serial]` gates correctly serialize env-mutating tests under concurrency). The trycmd cli binary now runs 6 cases.
